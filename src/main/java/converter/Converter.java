...
public class Converter {
    ...
    private static final double ro = (3600.0 * 180.0)/pi; //206264.8062; // Число угловых секунд в радиане
    ...
    public static double[] WGS84ToSK42Meters(double latWgs84, double longWgs84, double heightWgs84) {
        // Часть 1: Перевод Wgs84 географических координат(долготы и широты в градусах)
        // в СК42 географические координаты (долготу и широту в градусах)//Part 1:
        // Converting Wgs84 geographical coordinates(longitude and latitude in degrees)
        // to SK42 geographical coordinates(longitude and latitude in degrees)
        ...
        // Эллипсоид WGS84 (GRS80, эти два эллипсоида сходны по большинству
        // параметров)//Ellipsoid WGS84 (GRS80, these two ellipsoids are similar in most
        // parameters)
        ...
        // Линейные элементы трансформирования, в метрах//Linear transformation
        // elements, in meters
        double dx = 23.92;
        double dy = -141.27;
        double dz = -80.9;

        // Угловые элементы трансформирования, в секундах//Angular transformation
        // elements, in seconds
        double wx = 0;
        double wy = 0;
        double wz = 0;
        ...
        double dB = ro / (M11 + heightWgs84)
                * (N1 / a1 * e21 * Math.sin(B) * Math.cos(B) * da
                        + (Math.pow(N1, 2) / Math.pow(a1, 2) + 1) * N1 * Math.sin(B) * Math.cos(B) * de2 / 2
                        - (dx * Math.cos(L) + dy * Math.sin(L)) * Math.sin(B) + dz * Math.cos(B))
                - wx * Math.sin(L) * (1 + e21 * Math.cos(2 * B)) + wy * Math.cos(L) * (1 + e21 * Math.cos(2 * B))
        ...
        L = longWgs84 * Math.PI / 180;
        N1 = a1 * Math.pow((1 - e21 * Math.pow(Math.sin(B), 2)), -0.5);
        double dL = ro / ((N1 + heightWgs84) * Math.cos(B)) * (-dx * Math.sin(L) + dy * Math.cos(L))
                + Math.tan(B) * (1 - e21) * (wx * Math.cos(L) + wy * Math.sin(L)) - wz;

        double SK42_LongDegrees = longWgs84 - dL / 3600;// долгота в ск42 в градусах//longitude in sk42 in degrees

        // Часть 2: Перевод СК42 географических координат (широты и долготы в градусах)
        // в СК42 прямоугольные координаты (северное и восточное смещения в
        // метрах)//Part 2: Converting of SK42 geographical coordinates (latitude and
        // longitude in degrees) into SK42 rectangular coordinates(easting and northing
        // in meters)
        ...
        double Lon0 = (zone * 6 - 3) * Math.PI / 180; // Центральный меридиан (в радианах)//Central Meridian (in
                                                      // radians)
        double N0 = 0.0; // Условное северное смещение для начальной параллели//Conditional north offset
                         // for the initial parallel
        double E0 = zone * 1e6 + 500000.0; // Условное восточное смещение для центрального меридиана//Conditional
                                           // eastern offset for the central meridian

        // Перевод широты и долготы в радианы//Converting latitude and longitude to
        // radians
        double Lat = SK42_LatDegrees * Math.PI / 180.0;
        double Lon = SK42_LongDegrees * Math.PI / 180.0;

        // Вычисление переменных для преобразования//Calculating variables for
        // conversion
        ...
        double M2 = (3 * n + 3 * Math.pow(n, 2) + 21.0 / 8.0 * Math.pow(n, 3)) * Math.sin(Lat - Lat0)
                * Math.cos(Lat + Lat0);
        double M3 = (15.0 / 8.0 * Math.pow(n, 2) + 15.0 / 8.0 * Math.pow(n, 3)) * Math.sin(2 * (Lat - Lat0))
                * Math.cos(2 * (Lat + Lat0));
        ...
        double VI = v / 120 * Math.pow(cosLat, 5)
                * (5 - 18 * Math.pow(tanLat, 2) + Math.pow(tanLat, 4) + 14 * n2 - 58 * Math.pow(tanLat, 2) * n2);

        // Вычисление северного и восточного смещения (в метрах)//Calculation of the
        // north and east offset (in meters)
